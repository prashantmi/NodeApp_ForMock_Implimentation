...
function setEventListenersAfterReqnListTableDrawn(){
  ...
  if($('.addRemoveReqnBtn').length){
  	$('.addRemoveReqnBtn').off().on("click", function (){
...
function enableEntryModifyBtn(enableFlag){
	if(enableFlag==true){
		$('#enterResultBtn').attr({"disabled":false});
		  $('#modifyEntryBtn').attr({"disabled":false});
		  
		  tippyLoadingUtlInstance.enterResultBtnDiv.setContent('Click to Enter Result of Selected Requisition');
		  tippyLoadingUtlInstance.modifyEntryBtnDiv.setContent('Click to Modify Entry of Selected Requisition');
	}else{
		$('#enterResultBtn').attr({"disabled":true});
		$('#modifyEntryBtn').attr({"disabled":true});
...
var globalSearchParam={collAreaCode:"", labCode:"", resultEntryStatusCode:"", dateOrFilter:"", dateTypeCode:"",
...
function AjaxGetEntryReqnList(globalSearchParam){
	...
	var _mode = "AjaxGetEntryReqnList";
	  ...
	  + labCode + "&resultEntryStatusCode=" + resultEntryStatusCode + "&dateOrFilter=" + dateOrFilter + "&dateTypeCode=" + dateTypeCode + "&fromDate=" + fromDate
...
function openResultEntryIFrame(globalSelectedReqnData){
	...
	var requestData = {hmode:"GetReqnDetailsEditable", isPatDetailPage:"1", selectedCheckbox:concatSelectedReqnData, chkSendToMachine:concatChkSendToMachine, newEntry:valstusCod};
...
function setAddRemoveReqnBtnVal(data, type, row){
	...
	var addRemoveReqnBtnVal = {"crNo":row.crNo, "requisitionNo":row.requisitionNo, "requisitionDNo":row.requisitionDNo,
	   										"testCode":row.testCode, "groupCode":row.groupCode, "labCode":row.labCode };
...
function entryCustomData(col, noOfThPairInOneRow) {
	  var tbdat = '';
	    ...
	    tbdat = '</tr>'; entryCellCount = 0; isEntryFirstRow = 0;
	  }
